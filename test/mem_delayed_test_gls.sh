...
python toy_proc/run_yosys.py --no-cells --in-verilog src/mem_delayed_small.sv \
    src/assert_ignore.sv src/const.sv src/mem_delayed.sv \
    --top-module mem_delayed >/dev/null

cat <<EOF > build/insert_monitor.txt
  always @(*) begin
    \$monitor("t=%0d 4.v.always* rst=%0d ena=%0d rd_req=%0d wr_req=%0d addr=%0d rd_data=%0d wr_data=%0d busy=%0d ack=%0d clks_to_wait=%0d",
...
iverilog -Wall -g2012 tech/osu018/osu018_stdcells.v build/netlist/4.v src/const.sv src/assert.sv \
