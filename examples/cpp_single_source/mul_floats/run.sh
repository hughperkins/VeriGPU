...
${CLANGPP} -fPIE \
    ...
    -S ../mul_floats.cpp \
    -o mul_floats-hostraw.ll

# device-side => mul_floats.ll
${CLANGPP} -fPIE \
    -x cuda \
    ...
    -I${BASEDIR}/prot/verilator/prot_single_source \
    -S ../mul_floats.cpp \
    -o mul_floats-device.ll

${LLC_ZFINX} mul_floats-device.ll -o mul_floats-device.s --march=riscv32 -mattr=+m,+zfinx

# now we have to patch hostside...
${GPURUNTIMEDIR}/patch_hostside \
     --devicellfile mul_floats-device.ll \
     --deviceriscvfile mul_floats-device.s \
     --hostrawfile mul_floats-hostraw.ll \
     --hostpatchedfile mul_floats-hostpatched.ll
echo patched hostside

${LLC} mul_floats-hostpatched.ll --relocation-model=pic -o mul_floats-hostpatched.s

g++ -std=c++14 -fPIE -c mul_floats-hostpatched.s
g++ -std=c++14 -fPIE -I${VERILATORDIR}/include -c ${VERILATORDIR}/include/verilated.cpp

g++ -o mul_floats mul_floats-hostpatched.o -L${GPURUNTIMEDIR} -lverigpu_runtime ${LIBEXPFS}

set +x

./mul_floats +verilator+rand+reset+2 +verilator+seed+$(($RANDOM * 65536 + $RANDOM))
